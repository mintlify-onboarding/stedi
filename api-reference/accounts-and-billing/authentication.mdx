...
tags:
  - gettingStarted
description: How to create an API key and make authenticated API calls.
---
...
Your API key should be kept secret. Never share it or commit it to source control.
</Tip>

## Making authenticated API calls

When making an HTTP request to a Stedi API, pass your API key in the Authorization header, prefixed by the word `Key`. The following example shows a cURL request to [generate EDI](/core/parsing-and-generating-edi#stedi-http-api). The transactions array is abbreviated for readability.
